...
Schedule creation defaults to a six-hour interval (`creationCadenceMinutes=360`). Never claim that an existing active schedule changed until its live setting is verified. Each actual cadence defines its own report window; Story integration becomes due after 345 elapsed minutes so a nominal six-hour reservation tolerates scheduler delivery variance without becoming a briefing.
