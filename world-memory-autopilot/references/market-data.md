...
### Preferred live routes

Read Google Finance quote pages first for VIX9D, VIX, VIX3M and VIX6M at `https://www.google.com/finance/quote/{symbol}:INDEXCBOE`; beta redirects on the same host are accepted. Use web open (not search snippets or AI answer panels) and parse the quote header using `scripts/world_memory/google_finance.py:parse_quote`. Pass its deterministic output through the existing structured candidate/evidence binding and market validator. The closed observation has one sourceLocator: use the VIX quote page as a curve anchor, not as proof that every component came from that page. In Collection and Report sources-and-data prose, retain each parsed component page URL, observedAt with offset, displayedTimestamp and yearInferred flag alongside its level. The collector repeats the anchor locator; the per-component prose index supplies the actual page provenance. Do not add fields to the closed candidate or persist raw page text. Never use the related-assets panel, daily percent change or previous close as the quote. A missing chart does not invalidate a complete quote header.

Choose the request date from the actual common observed quote date, not today's calendar date. Retain the offset and the inferred-year flag when the header omits a year; the parser permits only an unambiguous preceding seven-day date. Treat prior-session data on a weekend as dated observations, never as today's real-time quote. Missing or differently dated components remain partial; fetch only missing same-date components from planned fallbacks. A complete validated Google observation skips all later providers including the spreadsheet. Keep the registry CSV address for compatibility and last-resort read-only fallback, without depending on it for routine success.

Use Alpaca `get_stock_bars` with explicit feed=iex, currency=USD, timeframe=1Day for HYG/LQD and RSP/SPY (a batched symbol request is allowed). Label IEX, use only completed daily bars and intersect observed dates; retain the existing 14-day/6-common-day and 45-day/21-common-day requests. Do not call ETF ratios measures of monetary liquidity: WALCL, WDTGAL and RRPONTSYD remain economic series with their own units and observation dates.
