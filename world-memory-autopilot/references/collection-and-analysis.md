...
Use the public pure helpers in `world_memory.notion_payloads` to assemble connector requests; they perform no external I/O. Build Collection writes with `collection_pages`, which returns ordered one-page requests containing at most 50 retained articles each. Submit those requests sequentially and collect every confirmed page ID; do not concatenate them or submit multiple generated Collection pages in one connector call. Relate `report_page` to all confirmed Collection page IDs. This keeps each Notion Markdown conversion safely below the prior 425-item processing limit while preserving every retained item exactly once. For a Story create, call `story_page`, submit that request, and only after the created Story is confirmed call `story_change_page` with its confirmed page ID. For an update, call `story_update` with the validated Story page ID, execute its two returned steps in order (`update_properties`, then `replace_content`), and call `story_change_page` only after both Story steps are confirmed. These payload builders are Python helpers, not additional CLI commands.
...
Do not call a separate LLM quality reviewer. Produce these sections inside the existing one temporary LLM plan, apply deterministic Markdown structure validation, and use only the existing maximum one contract-guided repair on invalid output. Meaning, importance, causality, and sufficiency remain model responsibilities inside that generation; deterministic code does not keyword-score them.
